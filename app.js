    ...
    const promises = Array.from(clipboardData.items).map(async (item) => {
        try {
            ...
            if (item.kind === 'file') {
                ...
                if (file.type.startsWith('image/')) return await processFile(file);
            }
...
organize.addEventListener('click', async function() {
  ...
  for (const item of images) {
    ...
    if (item.img instanceof HTMLElement) {
      ...
      clone.draggable = true; // Make draggable
      ...
      canvas.draggable = true; // Make draggable
    ...
    sizeInput.addEventListener('change', async function() {
      ...
      findBestJPEGQuality(targetCanvas, targetKB, (newBlob) => {
        ...
        if (currentCanvasOrImage instanceof HTMLImageElement) {
          ...
          // Re-attach drag event to the new element
    ...
    // Helper to attach drag-out functionality with custom filename
    function attachDragEvent(element) {
      element.addEventListener('dragstart', async function(e) {
        ...
        // If we don't have a currentBlob yet, generate one quickly
        const blob = currentBlob || await new Promise(resolve => targetCanvas.toBlob(resolve, 'image/jpeg'));
        
        // Grab the latest text from the name input field
        const finalFileName = nameInput.value.trim() || 'image.jpg';
        const file = new File([blob], finalFileName, { type: 'image/jpeg' });

        // Attach the file to the drag data transfer object
